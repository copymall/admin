        ...
        <el-form :model="homeAdvertise"
            ...
            <el-form-item label="广告备注：">
                <el-input
                    ...
                    :rows="5"
                    placeholder="请输入内容"
                    v-model="homeAdvertise.note"></el-input>
            </el-form-item>

            <el-form-item>
                <el-button type="primary" @click="onSubmit('homeAdvertiseFrom')">提交</el-button>
                <el-button v-if="!isEdit" @click="resetForm('homeAdvertiseFrom')">提交</el-button>
            </el-form-item>
        </el-form>
    </el-card>
</template>
    ...
    export default {
        ...
        date(){
            return{
                ...
                rule:{
                    name:[
                        {require:true,message:'请输入广告名称',trigger:'blur'},
                        {min:2,max:140,message:'长度在2到140个字符',trigger:'blur'}
                    ],
                    url:[
                        {require:true,message:'请输入广告连接',trigger:'blur'}
                    ],
                    startTime:[
                        {require:true,message:'请选择开始时间',trigger:'blur'}
                    ],
                    endTime:[
                        {require:true,message:'请选择到期时间',trigger:'blur'}
                    ]
                },
            }
        },
                    ...
                    if(valid){
                        this.$confirm('是否提交数据','提示',{
                            ...
                            if(this.isEdit){
                                ...
                                createHomeAdvertise(this.homeAdvertise).then(responnse=>{
                                    this.$refs[formName].resetFields();
                                    this.homeAdvertise=Object.assign({},defaultHomeAdvertise);
                                    this.$message({
                                        message:'提交成功',
